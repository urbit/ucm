            ...
            <Box sx={{ flexGrow: '1', overflow: 'hidden' }}>
              <Switch>
                ...
                <Route
                  path="/chat"
                  nest
                  component={() => ChatPage({ site, group, chats: channels.chats })}
                />
                <Route
                  path="/forum"
                  nest
                  component={() => ForumPage({ site, group, forums: channels.forums })}
                />
                <Route
                  path="/blog"
                  nest
                  component={() => BlogPage({ site, group, meta: channels.blog! })}
                />
                <Route path="/radio" component={Radio} />
                <Route path="/search" nest component={() => SearchPage({ site })} />
                <Route path="/user/:patp">
                  {(params) => {
                    const patp = params.patp as Ship;
                    if (clientShip.length > 28) return <NotFound err="Not logged in" />;
                ...
                <Route path="/:page">
                  {(params) => {
                    ...
                    if (!text) return <NotFound err="page not found" />;
                      ...
                      return (
                        <StaticPage title={params.page} markdown={text} site={site} />
                      );
                  }}
                </Route>
              </Switch>
