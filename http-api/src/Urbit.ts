...
export class Urbit {
  ...
  private async ninjaJson(proxyApp: string, ...json: Message[]): Promise<void> {
    // TODO pass the channel?
    const response = await this.fetchFn(`${this.url}/${proxyApp}`, {
      ...this.fetchOptions,
      method: 'PUT',
  ...
  async ninjaPoke<T>(params: PokeInterface<T> & { desk: string, proxyApp: string }): Promise<number> {
    const { app, desk, mark, json, ship, onSuccess, onError, proxyApp } = {
      onSuccess: () => { },
      onError: () => { },
      ship: this.ship,
      ...params,
