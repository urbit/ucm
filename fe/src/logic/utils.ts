...
import { RADIO_SHIP, REF_REGEX, SHIP_REGEX, WIKI_SHIP } from "./constants";
...
export function enkebab(s: string) {
  return anyAscii(s.trim())
    .toLowerCase()
...
export async function doubleCheckApps(
  apps: Apps,
  chans: Channels,
  pikes: Pikes,
): Promise<Apps> {
  const blog = chans.blog && apps.blog ? chans.blog.name : apps.blog;
  const chat = chans.chats.map((c) => c.title);
  const forum = chans.forums.map((c) => c.title);
  const radio = pikes?.radio?.sync?.ship === RADIO_SHIP && apps.radio;

  return { blog, chat, forum, radio, wiki: apps.wiki, static: apps.static };
}
export async function checkWikis(pikes: Pikes): Promise<string[]> {
  const installed = pikes?.wiki?.sync?.ship === WIKI_SHIP;
  if (!installed) return [];
  else {
    // const wikires = await fetch("/wiki"); // wtf man give me scry endpoints
    // const wikires = await fetch("http://localhost:8081/wiki"); // wtf man give me scry endpoints
    // const html = await wikires.text();
    // const parser = new DOMParser();
    // const parsed = parser.parseFromString(html, "text/html");
    // console.log(parsed, "fucking wiki frontend");
    return [];
  }
}
