...
function lexerToTlonTop(acc: Verse[], m: Token): Verse[] {
  ...
  if (m.type === 'blockquote')
    return [...acc, { inline: [{ blockquote: m.tokens!.map(lexerToTlon).flat() }] }];
...
function lexerToTlon(m: Token): Inline[] {
  ...
  else if (m.type === 'em') return [{ italics: m.tokens!.map(lexerToTlon).flat() }];
  else if (m.type === 'strong') return [{ bold: m.tokens!.map(lexerToTlon).flat() }];
  else if (m.type === 'del') return [{ strike: m.tokens!.map(lexerToTlon).flat() }];
  else if (m.type === 'blockquote')
    return [{ blockquote: m.tokens!.map(lexerToTlon).flat() }];
  else if (m.type === 'link') return [{ link: { href: m.href, content: m.text } }];
  else if (m.type === 'image') return [{ link: { href: m.href, content: m.text } }];
  ...
  else if (m.type === 'html' || m.type === 'heading') return parsePlainText(m.text);
  else return [m.raw];
}
  ...
  if (!matches) return data;
    ...
    return matches.reduce((acc, item) => {
      ...
      if (isValidPatp(item.trim()))
        return [acc[0].replace(item, uid), [...acc[1], [uid, { ship: item.trim() }]]];
      else return acc;
    }, data);
}
