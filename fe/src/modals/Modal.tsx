...
export default function MuhModal({ children }: { children: ReactNode }) {
  ...
  function clickAway(e: React.MouseEvent) {
    e.stopPropagation();
    if (!modalRef.current || !modalRef.current.contains(e.target as any)) setModal(null);
  }
...
export function StaticPageModal({
  ...
  async function del() {
    ...
    const order = site['app-order'].filter((ao) => ao !== `static/${name}`);
    const ns = { ...site, apps, 'app-order': order };
    await createSite(ns);
    sync().then((_) => setModal(null));
  }
  useEffect(() => {
